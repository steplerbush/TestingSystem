...
public class LogoutCommand implements ICommand {
    ...
    /**
     * Log out user from the system, invalidates user's session and redirects
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (session != null) {
            logger.info("Session invalidate for user with role = '"
                    + session.getAttribute(AttributesManager.
                            ATTRIBUTE_USER_ROLE) + "' and login = '"
                    + session.getAttribute(AttributesManager.
                            PARAM_NAME_LOGIN) + "'" + ". RemoteAddr: "
